...
export class IdeaService {
  ...
  private smsCollection: AngularFirestoreCollection<any>;
 
  constructor(private afs: AngularFirestore) {
    this.ideaCollection = this.afs.collection<Idea>('ideas');
    this.smsCollection = this.afs.collection<Idea>('sms');
    ...
    this.sms = this.ideaCollection.snapshotChanges().pipe(
      map(actions => {
        return actions.map(a => {
          const data = a.payload.doc.data();
          const id = a.payload.doc.id;
  ...
  addSMS(sms: any): Promise<DocumentReference> {
    return this.smsCollection.add(sms);
  }
